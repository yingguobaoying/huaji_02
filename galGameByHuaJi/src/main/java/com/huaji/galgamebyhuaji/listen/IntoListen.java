...
public class IntoListen {
    private final VignaChatClientConfig aiConfig;
    ...
    public void onContextRefreshed(ContextRefreshedEvent event) throws Exception {
        ...
        if (event.getApplicationContext().getParent() == null) {
            ...
            if (i == 430 || i == 43 || i == 4 || i == 3 || i == 12 || i == 142 || i == 14 || i == 1) {
                ...
                System.err.println("=========================================");
                log.info("=========================================");
                log.info("==============  红豆可爱滴捏~~~~  =========");//如果后续我找别人运维的话希望他不要被这玩意吓死
                log.info("=========================================");
                log.warn("*****************************************");//为什么用warn因为红色好看(误)
                log.warn("************  红豆可爱滴捏~~~~  ***********");
                log.warn("*****************************************");
                log.error("=========================================");//为什么用error因为深红色好看(误)
                log.error("============  红豆可爱滴捏~~~~  ===========");
                log.error("=========================================");
            }
            BackgroundImgController.updateFileNameList(new File(FileUtil.formatUrl(Constant.getRESOURCE_SAVE_PATH(), FileCategory.IMG.getFILE_SAVE_URL(), "background")));
    ...
    public void onContextClosed(ContextClosedEvent event) {
        ...
        log.info("{}服务器关闭", TimeUtil.getVisualDateFormatTime(new Date()));
        log.info("在服务器关闭时,使{}位在线用户离线", i);
        System.err.println("在服务器关闭时,牢大肘击了" + i + "个倒霉用户");
        log.warn("在服务器关闭时,牢大肘击了{}个倒霉用户", i);
        VignaHttpClientFactory.shutdown();
    }
}
