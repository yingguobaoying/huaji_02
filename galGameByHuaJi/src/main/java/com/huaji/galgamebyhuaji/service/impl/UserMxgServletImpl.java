		...
		return GlobalLock.safeExecute(userId, () -> {
			if ( banHeadPortrait ) {
				...
				if ( !MyStringUtil.isNull(users.getUserHeadPortraitUrl()) )
					fileServlet.deleteFile(users.getUserHeadPortraitUrl(), String.valueOf(new File(Constant.RESOURCE_SAVE_PATH, "/img")));
				Users u = new Users();
				u.setUserId(userId);
			...
			try {
				jpeg = fileServlet.uploadFile(croppedImage, "jpeg", userToken, name);
			} catch ( IOException | BestException e ) {
				throw new RuntimeException("上传头像失败", e);
			}
			if ( jpeg.isOperationResult() ) {
				try {
					WriteError.tryWrite(usersMapper.setUserHeadPortraitUrl(userId, jpeg.getReturnResult()));
				} catch ( WriteError e ) {
					MyLogUtil.error(UserMxgServletImpl.class, e);
					fileServlet.deleteFile(name, String.valueOf(new File(Constant.RESOURCE_SAVE_PATH, "/img")));
