...
public class LoginServiceIMPL implements LoginService {
	...
	public UserToken login (UsersWithBLOBs users, HttpServletRequest request, Long KeepTime) throws BestException {
		...
		if ( login.size() != 1 ) throw new UserException("系统检测到账户信息存在异常，为确保您的账户安全，请稍后重试或联系站长。", login, ErrorEnum.USER_REPEAT_ERROR);
	...
	public ReturnResult<Users> testRegisterMxg (Users users, Integer testUserId) {
		...
		if ( MyStringUtil.isNull(users.getUserNameLogin()) ) throw new OperationException("用户名不可为空，请设置您的昵称或用户名。");
		...
		for ( Users u : users1 ) {
			if ( isLoginName && u.getUserNameLogin().equals(users.getUserNameLogin()) ) {
				isLoginName = false;
				returnUser.setUserNameLogin(u.getUserNameLogin());
			}
			if ( isUserEmail && u.getMailbox().equals(users.getMailbox()) ) {
				isUserEmail = false;
				returnUser.setMailbox(u.getMailbox());
			}
			if ( isUserPE && u.getUserPe().equals(users.getUserPe()) ) {
				isUserPE = false;
				returnUser.setUserPe(u.getUserPe());
			}
	...
	public String ClockIn (Integer userId) throws SessionExceptions {
		...
		if ( userId == null ) throw new SessionExceptions("用户信息不存在或已被删除，请重新登录。", ErrorEnum.SESSION_NOT_AVAILABLE_ERROR);
