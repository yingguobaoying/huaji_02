...
public class LoginServiceAspect {
    ...
    private String buildEndLogMessage(String methodName, Object result, boolean success, 
        ...
        if (success && result != null) {
            sb.append(" | 结果: ");
            if (result instanceof UserToken token) {
                sb.append("UserToken[userId=").append(token.getUserId())
                  .append(", token=***]");
            } else if (result instanceof ReturnResult returnResult) {
                sb.append("ReturnResult[success=").append(returnResult.isOperationResult())
                  .append(", message=").append(returnResult.getReturnMsg()).append("]");
            } else if (result instanceof Users user) {
                sb.append("Users[userId=").append(user.getUserId())
